...
class SolutionGenDependencyError(Exception):
    """Erro de dependência crítica para geração de solução na nuvem Azure."""
    pass
...
def get_solution_prompt_azure(transcription_text):
    ...
    """
    prompt = f"""
    Você é um arquiteto de soluções Azure. Analise o seguinte texto:
    {transcription_text}
...
def call_gemini_api_azure(prompt, description, api_key):
    """
    Executa chamada à API Google Gemini para geração de solução para Azure.
